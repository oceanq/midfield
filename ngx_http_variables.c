...
static ngx_int_t ngx_http_variable_remote_port(ngx_http_request_t *r,
    ngx_http_variable_value_t *v, uintptr_t data);
static ngx_int_t ngx_http_variable_sslvpn_host(ngx_http_request_t *r,
    ngx_http_variable_value_t *v, uintptr_t data);
static ngx_int_t ngx_http_variable_sslvpn_hostname(ngx_http_request_t *r,
    ngx_http_variable_value_t *v, uintptr_t data);
...
static ngx_int_t
ngx_http_variable_sslvpn_host(ngx_http_request_t *r,
    ngx_http_variable_value_t *v, uintptr_t data)
{
    v->len = 0;
    v->valid = 1;
	...
	// uri.data like: /sslvpn/http/10.2.1.28/mm/cc.html
	ret = sscanf(r->uri.data,"/sslvpn/%5[^/]/%239[^/]/",proto,host);
	if(ret!=2) {
		printf("ret %d\n",ret);
		return NGX_ERROR;
	}
	if(memchr(host,':',sizeof(host))) {
		v->len =  ngx_sprintf(v->data, "%s://%s/", proto,host) - v->data;
	}
	else {
		if(!ngx_strcmp(proto,"http")) {
			v->len =  ngx_sprintf(v->data, "%s://%s:80/", proto,host) - v->data;
		}
		else if(!ngx_strcmp(proto,"https")) {
			v->len =  ngx_sprintf(v->data, "%s://%s:443/", proto,host) - v->data;
		}
		else {
			return NGX_ERROR;
		}
...
static ngx_int_t
ngx_http_variable_sslvpn_hostname(ngx_http_request_t *r,
    ngx_http_variable_value_t *v, uintptr_t data)
{
    v->len = 0;
    v->valid = 1;
...
static ngx_int_t
ngx_http_variable_sslvpn_path(ngx_http_request_t *r,
    ngx_http_variable_value_t *v, uintptr_t data)
{
    v->len = 0;
    v->valid = 1;
